...
const QueueManagement: React.FC<QueueManagementProps> = ({ isAgentView = false }) => {
  ...
  const generatePrintContent = (ticket: TicketData): string => {
    ...
    const priorityBg = ticket.priorite === 'critique' ? '#fee2e2' : ticket.priorite === 'urgent' ? '#ffedd5' : '#dbeafe';
    const priorityColor = ticket.priorite === 'critique' ? '#991b1b' : ticket.priorite === 'urgent' ? '#9a3412' : '#1e40af';
...
<body>
  <div class="ticket">
    ...
    \${ticket.priorite !== 'normal' ? \`<div class="priority">\${ticket.priorite === 'critique' ? '⚠️ PRIORITÉ CRITIQUE' : '⚡ PRIORITÉ URGENTE'}</div>\` : ''}
  ...
  // Ticket Component - Display Only (not for printing)
  const Ticket = () => {
